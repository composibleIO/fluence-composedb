...
import { writeEncodedCompositeRuntime, readEncodedComposite } from '@composedb/devtools-node';
import { newDid } from '../utils/did.js';
import { JSONToBase64url } from '../utils/serialize.js';
export const compositeCompile = async (ceramic, name) => {
    const client = new CeramicClient(ceramic);
    return await writeEncodedCompositeRuntime(client, './composites/' + name + '.json', './' + name + '--runtime.js');
};
export const compositeDeploy = async (ceramic, name) => {
...
export const compositeDefinition = async (ceramic, name, serialized) => {
